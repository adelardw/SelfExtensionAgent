...
from src.improve import get_prompt as get_prompt_override, maybe_auto_improve, maybe_improve_user
from src.improve.safety import sanitize_tool_output, strip_ungrounded_pii
from src.improve.prompt_store import format_fewshots, add_fewshot, add_user_fewshot
from src.external import get_external_context, format_external_context
from src.data.mcp_client import suggest_server, get_mcp_tools, discover_mcp, approve_server, try_connect_discovered
from src.graph.subagents import get_subagent_tools
from src.memory.memory_tools import make_memory_tools, clear_scratch
from src.memory import project_memory
from src.runtime import context_files
...
from src.llm.llm import chat as _chat
...
async def synthesize_node(state: GeneralGraphState) -> dict:
    ...
    # Каскад-синтез: финал собирает УМНЫЙ ярус (deep) на сжатом дистилляте. Без своего таймаута —
    # LLM-вызов ограничен SDK-таймаутом клиента; упал → guard ниже подхватит лучший шаг.
    try:
    ...
    # ФАЙЛ ПРОСИЛИ, НО ЕГО НЕТ (шаг экспорта не запустился или упал): формируем .xlsx из УЖЕ СОБРАННЫХ
    # данных прямо здесь, чтобы пользователь получил настоящий файл, а не обещание. Только реальные
    # значения из собранного (модели запрещено выдумывать).
    if _wants_file_output(state.get("query", "")) and not run_context.artifacts():
        try:
            _ex = await asyncio.wait_for(llm.ainvoke(
                "Извлеки из собранных данных ниже таблицу в формате CSV (первая строка — заголовки, "
                "разделитель — запятая). Только РЕАЛЬНЫЕ значения из данных, без выдумок, без "
                "пояснений и без markdown. Если табличных данных нет — верни пустую строку.\n\n"
                "Собранные данные:\n" + results_text[:12000]),
                timeout=float(config.agent.get("llm_timeout", 60)))
            _csv = (_ex.content if hasattr(_ex, "content") else str(_ex)).strip()
            _csv = re.sub(r"^```[a-zA-Z]*\n?|\n?```$", "", _csv).strip()
            if _csv and "," in _csv and "\n" in _csv:
                import csv as _csvmod, io as _io
                _rows = [r for r in _csvmod.reader(_io.StringIO(_csv)) if any(c.strip() for c in r)]
                if len(_rows) >= 2:
                    from src.runtime.artifacts import write_table
                    write_table("data", _rows[0], _rows[1:], "Данные")  # настоящий .xlsx
        except Exception:  # noqa: BLE001
            pass
...
async def validation_node(state: GeneralGraphState) -> dict:
    ...
    # БЮДЖЕТ УБРАН → «оборван» означает только абсолютный анти-runaway потолок шагов (патология).
    # Не «бюджет/время». План доводится до естественного конца.
    _cut = state.get("steps_executed", 0) >= MAX_STEPS_HARD
    ...
    if flag_false_refusal:
        ...
        # результат/файл), а не теряем; иначе честно говорим, что часть данных не нашлась.
        draft = (state.get("final_answer") or "").strip()
        note = ("Доступ к вебу есть — дело в наличии самих данных, не в доступе. Что нашёл — собрал; "
                "если чего-то не хватает, скажи, что доработать.")
        if len(draft) >= 200 and not flag_meta_stub:
            final = draft + "\n\n———\n" + note
        else:
            final = "Часть данных по этому запросу собрать не удалось. " + note
        return {"validation_passed": True, "confidence": LOW_CONF,
                "final_answer": final,
...
def route_after_step(state: GeneralGraphState) -> str:
    ...
    ЕСТЕСТВЕННОГО конца — никаких «не довёл по бюджету», в т.ч. на ДОЛГИХ задачах. Единственный
    ранний выход — абсолютный анти-runaway потолок шагов (патология, не бюджет; план до него не доходит)."""
