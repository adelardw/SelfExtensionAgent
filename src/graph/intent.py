...
from src.memory.embedder import build_embedder, cosine
...
LABELS = ("web_grounding", "physical_browser", "play_media", "media_control", "self_contained")
...
_SEED: dict[str, list[str]] = {
    ...
    "web_grounding": [
        ...
        "свежие новости про ИИ сегодня", "latest news about AI today",
        ...
        "近くの安いラーメン屋はどこ", "ビットコインの現在価格はいくら",  # JA: где дешёвый рамен / цена биткоина
        "附近哪里能买到便宜的耳机", "今天的金价是多少",                    # ZH: где купить наушники / цена золота
        "أين يمكنني شراء لابتوب رخيص", "ما هو سعر الذهب اليوم",           # AR: где купить ноутбук / цена золота
        "근처 약국 영업시간 알려줘", "가장 좋은 노트북 추천해줘",          # KO: часы аптеки / лучший ноутбук
        "सबसे अच्छा स्मार्टफोन कौन सा है", "en yakın eczane nerede",     # HI лучший смартфон / TR ближайшая аптека
    ],
    ...
    "physical_browser": [
        ...
        "メールにログインして受信箱を開いて", "カートに追加して購入手続きへ",   # JA: войти в почту / в корзину и оформить
        "打开我的邮箱并登录", "把这个商品加入购物车",                          # ZH: открой почту и войди / в корзину
        "افتح بريدي وسجّل الدخول", "أضف هذا إلى سلة التسوق",                 # AR: открой почту и войди / в корзину
        "내 계정에 로그인해줘", "장바구니에 담고 결제 진행해줘",               # KO: войди в аккаунт / в корзину и оплати
    ],
    ...
    "play_media": [
        ...
        "ラジオヘッドの曲を再生して", "映画デューンを再生して",               # JA: включи radiohead / фильм дюна
        "播放一些爵士乐", "播放下一首歌",                                     # ZH: включи джаз / следующую песню
        "شغّل بعض موسيقى الجاز", "شغّل الفيديو على يوتيوب",                  # AR: включи джаз / видео на ютубе
        "재즈 음악 틀어줘", "다음 곡 재생해줘",                                # KO: включи джаз / следующий трек
    ],
    # УПРАВЛЕНИЕ плеером (пауза/стоп/громкость) — НЕ запуск: действие на странице, но БЕЗ
    # авто-дожима плея (отдельно от play_media, чтобы не путать «пауза» с «включи»).
    "media_control": [
        "поставь на паузу", "pause the music", "приостанови воспроизведение",
        "останови видео", "stop the video", "сделай тише", "turn the volume down",
        "сделай погромче", "выключи звук", "mute the sound", "pausa la reproducción",
        "mets en pause", "mach das video aus",
        "音楽を一時停止して", "音量を下げて",                                  # JA: пауза музыки / тише
        "暂停播放", "把音量调小一点",                                         # ZH: пауза / тише
        "أوقف الموسيقى مؤقتا", "اخفض الصوت",                                # AR: пауза музыки / тише
        "잠깐 멈춰줘", "소리 줄여줘",                                          # KO: пауза / тише
    ],
    ...
    "self_contained": [
        ...
        # СОЗДАНИЕ артефактов (код/приложение/презентация/документ) — это deliberate+инструменты,
        # НЕ веб-поиск и НЕ отказ; пол web_grounding не должен их угонять.
        "создай веб-приложение", "помоги мне сделать приложение", "create a web app",
        "напиши код приложения", "сделай мне сайт", "build me a landing page",
        "сделай презентацию про машинное обучение", "сделай презентацию", "create a presentation about ai",
        "составь документ", "напиши отчёт по проекту", "write a report", "make me a slide deck",
        "アプリを作って", "做一个网页应用", "أنشئ تطبيق ويب", "앱을 만들어줘",  # JA/ZH/AR/KO: создай приложение
        "почему небо голубое", "why is the sky blue", "pourquoi le ciel est bleu",
        "再帰とは何か説明して", "17かける23はいくつ",                          # JA: объясни рекурсию / 17×23
        "解释一下什么是递归", "写一个计算阶乘的函数",                          # ZH: объясни рекурсию / факториал
        "اشرح كيف يعمل الفرز السريع", "كم يساوي 17 في 23",                  # AR: объясни quicksort / 17×23
        "재귀가 뭔지 설명해줘", "피보나치 함수를 작성해줘",                    # KO: объясни рекурсию / фибоначчи
    ],
}
