...
async def reflexion_node(state: GeneralGraphState) -> dict:
    ...
    # Неоднозначно (clarify ИЛИ ambiguity выше гейта) → СТРУКТУРНЫЙ батч уточнений (clarify_gate,
    # мультиселект, ответ привязан к ходу), а не переспрос прозой. Затем — исполнение.
...
# ══════════════════════════════════════════════════════════════════════════
# ЭКСПЕРИМЕНТ (config: experimental.composer) — мета-контроллер композиции примитивов
# ──────────────────────────────────────────────────────────────────────────
# Вместо «reflexion выбирает 1 из 6 режимов-шаблонов» — цикл, где контроллер на
# КАЖДОМ шаге выбирает СЛЕДУЮЩИЙ когнитивный примитив, глядя на накопленные
# промежуточные результаты, и сам решает, когда финализировать. Режимы fast/
# reason/act/… в этой картине — частные «запечённые» рецепты композиции.
# Изолировано: рабочий граф (src/agent.py) НЕ затронут; включается флагом.
# ══════════════════════════════════════════════════════════════════════════
...
async def _composer_pick(query: str, mem: str, scratch: list[dict], steps_left: int) -> _ComposerStep:
    ...
    try:
        d = await llm.with_structured_output(_ComposerStep).ainvoke(
...
async def _prim_recall(goal: str, state: GeneralGraphState) -> str:
    try:
        ...
        qv = await memory_store.embedder.aembed(goal) if memory_store.embedder.enabled else None
        txt, _ = memory_store.recall_scored(uid, goal, qvec=qv)
...
async def _prim_verify(goal: str, query: str, scratch: list[dict], deadline: float) -> str:
    sys = ("Ты — критик-верификатор. Проверь собранные результаты на противоречия, пробелы и "
           "невалидные утверждения относительно запроса. Кратко: что подтверждено, что под "
           "вопросом, чего не хватает.")
    prompt = f"ЗАПРОС: {query}\n\nСОБРАНО:\n{_scratch_text(scratch)}\n\nФОКУС ПРОВЕРКИ: {goal}"
    out, _ = await _exec_compose(sys, prompt, deadline)
    return out
...
async def composer_node(state: GeneralGraphState) -> dict:
    """ЭКСПЕРИМЕНТ: мета-контроллер компонует примитивы под задачу (вместо выбора режима)."""
    query = state["query"]
    mem = state.get("memory_context", "")
    qe = state.get("query_emb") or None
    max_steps = int(config.get("experimental", {}).get("max_steps", 12))
    scratch: list[dict] = []
    trace: list[str] = []
    for i in range(max_steps):
        deadline = min(STEP_DEADLINE_CAP, max(15.0, MAX_RUN_SECONDS - runbudget.elapsed()))
        d = await _composer_pick(query, mem, scratch, max_steps - i)
        ...
        if d.primitive == "recall":
            ...
            out = await _prim_verify(d.goal, query, scratch, deadline)
        else:  # reason
            out = await _prim_reason(d.goal or query, query, scratch, deadline)
        scratch.append({"primitive": d.primitive, "goal": d.goal, "output": out})
    deadline = min(STEP_DEADLINE_CAP, max(15.0, MAX_RUN_SECONDS - runbudget.elapsed()))
    answer = await _composer_finalize(query, mem, scratch, deadline)
    grounded = query + "\n" + (mem or "") + "\n" + _scratch_text(scratch)
    answer = strip_ungrounded_pii(answer, grounded)
    print(f"[Composer] {' → '.join(trace)}")
...
def build_graph(checkpointer=None) -> CompiledStateGraph:
    ...
    Память (recall) и обучение (reflect) переиспользуются из рабочего агента; заменён
    лишь весь средний слой выбора режима — его собирает composer_node на ходу."""
...
def _build_graph_modes(checkpointer=None) -> CompiledStateGraph:
    """Оригинальный граф «1 из 6 режимов» — сохранён для сравнения (composer vs modes)."""
    graph = StateGraph(GeneralGraphState)
