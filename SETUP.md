...
Свой навык можно отдать готовым — он проходит **проверку ПЕРЕД приёмом**, недоверенный код
не исполняется в процессе агента:
```bash
python -m src.skills_import path/to/my_skill        # папка
python -m src.skills_import path/to/my_skill.zip    # или zip
```
Формат: каталог `<name>/` с `<name>.py` (функции `@tool`) и опц. `<name>.md` (инструкция).
Пайплайн: карантин (анти-zip-slip) → **AST-гейт** (статика: `os.system`/`eval`/`subprocess`… режутся)
→ **smoke-импорт в изолированном subprocess** (rlimits; должен грузиться и иметь ≥1 `@tool`) →
вердикт: **только при успехе** копируется в `src/skills/` и пишется в `registry.json`. Иначе —
отказ с причиной, ничего не ставится.
