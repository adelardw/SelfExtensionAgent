...
- **Conditional recall + GraphRAG-lite**: persona facts always, associative memory by a relevance gate (`recall_gate`); fact↔fact densify + spreading-activation; the query is embedded once and reused (no N HTTP calls).
...
- **Desktop + cross-platform packaging** — not started: a native binary/app per OS (.exe/.app/AppImage) and/or a desktop shell over the FastAPI brain. Approach (PyInstaller vs Tauri/Electron, local brain vs thin client to a remote server) is still open.
- **Provider settings — partial**: the `/config` panel + `config.local.yml` already persist provider/model/modes/grants. STILL missing: editing `base_url` and entering the **API key** from the settings UI (the key is env-only today: `OPEN_ROUTER_API_KEY`), and live key validation.
...
- **Десктоп + кроссплатформенная упаковка** — не начато: нативный бинарь/приложение под каждую ОС (.exe/.app/AppImage) и/или десктоп-оболочка поверх FastAPI-мозга. Подход (PyInstaller vs Tauri/Electron, локальный мозг vs тонкий клиент к удалённому серверу) — открыт.
- **Настройки провайдера — частично**: панель `/config` + `config.local.yml` уже персистят провайдер/модель/режимы/гранты. ОСТАЁТСЯ: правка `base_url` и ввод **API-ключа** из UI настроек (ключ сейчас только из env: `OPEN_ROUTER_API_KEY`) и живая валидация ключа.
