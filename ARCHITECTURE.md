...
**обучаемая программа**: запуск = forward pass (трейс активаций), а self-learning —
...
START
 ...
 + AutoRAG (БЗ пользователя + вложения сессии, BM25 + sanitize) + сброс журнала взаимодействий.
 ...
 (+ априорная оценка бандита: Beta/Thompson по похожим эпизодам пользователя, видит и неудачи;
 ...
 запуска, переиспользуются decompose/step/synthesize; нет ответа
...
| Память | `memory/store.py` (SQLite, WAL + пер-поточный conn), `embedder.py`, `vector_index.py` (TurboVec), `feedback.py`, **`memory_tools.py`**, **`interaction.py`** | эпизоды/факты(+тэги)/выводы/цели/саммари + граф-рёбра (prune теперь чистит и повисшие рёбра + капает рецепты); **УСЛОВНЫЙ И ГИБКИЙ recall** (`recall_scored`+гейт `recall_gate`: И факты, И ассоциативная память отбираются по релевантности к запросу — «recall не всегда», task-факты не текут в несвязанные запросы; граф-boost переживает гейт); **GraphRAG-lite** (`_densify_fact` fact↔fact по cosine + `_graph_boost` spreading-activation от релевантных эпизод-сидов, per-user, PII-контейнмент); запрос эмбеддится ОДИН раз (qvec прокидывается, не N HTTP-вызовов); implicit feedback. **Журнал взаимодействий**: HITL/clarify переживают запуск → эпизод + harvest без LLM. **Память-как-tool (3 яруса)**: `search_memory` / `recall_history` / `note_to_self`. **Авто-память скоупится по чату** (`_mem_scope`=session_id): facts/goal/episodes/summary — по треду (новый чат стартует чистым, цель не течёт между чатами); per-USER амортизация (few-shots/рецепты/привычки) — кросс-сессионно (write-key=read-key проверены) |
| Маршрутизация интентов | **`intent.py`** · `eval/route_eval.py` | **универсальный embedding-kNN роутер** (любой язык): кодбук маршрутов {web_grounding/physical_browser/play_media/media_control/self_contained}, cosine-kNN; **полностью заменил регулярные выражения по русскому языку — регулярные выражения маршрутизации УДАЛЕНЫ**, все сигналы идут только через классификатор. 5-й лейбл media_control (пауза/стоп/громкость) отделён от play_media, чтобы «поставь паузу» не запускало воспроизведение. Переиспускает query-эмбеддинг из recall (0 лишних вызовов), ФИКСИРОВАН по модели (тег+инвалидация). **Растёт из фидбек-лупа** (валидированный запуск→маршрут); per-label порог (`web_grounding=0.23`, тюнинг по confusion). КОРПУС pos/neg (`route_examples.db`) для будущего обучения локального head. Стат-оценка route_eval: **570 кейсов, 89.3%** [Wilson 86.5–91.6%] (media_control 93% · physical 96% · self_contained 91% · web_grounding 86% · play_media 80%, мультиязык вкл. JA/ZH/AR/KO/HI) |
...
| Самообучение / амортизация | `improve/`, **`habits.py`**, **`bandit.py`**, **`collective.py`**, `memory/store.py: recipes` | forward-харвест few-shots (глоб+**пер-пользователь**, двухъярусно с baseline); backward: дифф-credit-assignment → per-node gradients → оптимизация промптов; **per-user backward** (`graph_backward_user`: уроки из неудач пользователя → его few-shots); **измеримый accept/revert** (запуск ДО/ПОСЛЕ на кейсах) → ParamStore; **привычки** (`habits.py`: k похожих успешных дорогих запусков → факт-директива → router создаёт навык → привычка закрывается ); **априорная оценка режима, бандит** (`bandit.py`: Beta/Thompson по похожим эпизодам пользователя, видит и НЕУДАЧИ — в few-shots их нет; априорная оценка в memory_context reflexion, не диктат) |
...
~постоянна. Здесь каждый успешный запуск оставляет артефакт, делающий похожие задачи
...
тяжёлого/мультишагового запуска `reflect` сжимает работу (шаги + итог) в выжимку и добавляет в
...
добытый отрицательными запусками №1/№3: артефакт опыта должен **ЗАМЕНЯТЬ LLM-работу**
(zero-LLM селектор/декомпозиция), а не аннотировать её — подсказки/few-shots/априорные оценки раздувают
контекст всех вызовов и покупают только надёжность. Оговорки: это ОДИН парный запуск cold→warm
...
best-practice инсталляции с отпечатком профиля источника; похожим пользователям — рекомендация
...
- **Forward**: каждый запуск пишет `run_id`+активации нод в трейс и эпизод (исход+confidence).
 Принятые обдуманные запуски → few-shots (генерализация без LLM). **Векторизация под
...
- **Backward** (`improve/graph_learn.py`):
 ...
 3. `optimize_role` для КАЖДОЙ виноватой ноды → проверка плейсхолдеров + LLM-судья + **измеримый ДО/ПОСЛЕ** (запуск на кейсах неудач, сохраняем только при реальном улучшении, иначе откат) → `ParamStore`.
- **Per-user backward** (`graph_backward_user`): из неудач КОНКРЕТНОГО пользователя + того, КТО он (профиль/роли), синтезирует корректирующие УРОКИ → его персональные few-shots (ядро заморожено; пишет только в стор этого пользователя). Триггер при per-user деградации. Это «оптимизация под пользователя» как метод.
...
- **Политика оптимизации** (что backward вправе менять):
 ...
 - основной канал улучшения/персонализации — **few-shots** (глобальные + пер-пользователь);
...
- **Десктоп** — `desktop.py` поднимает `uvicorn src.interface.server:app` в daemon-потоке и открывает нативное
 ...
 строит **тот же граф** и отдаёт `/` (web-UI), `/chat`→`/run/{id}` (фоновый запуск через `astream`,
...
- `python scripts/amortize_bench.py` — проверка тезиса амортизации (платный живой запуск).
...
- **Интерактивный HITL/clarify по HTTP** — СДЕЛАНО: `/chat` запускает фоновый запуск (astream), клиент поллит `/run/{id}`; `clarify.set_clarifier`/`hitl.set_confirmer` всплывают как карточка Q/A-мультиселект и карточка подтверждения; ответ в `/run/{id}/respond`, запуск продолжается. Живой прогресс по узлам стримится; ответы на уточнения дедупятся в рамках запуска.
- **Импорт внешних навыков (проверка до приёма)** — СДЕЛАНО (`skills_import.py`): папка/zip пользователя → карантин (анти-zip-slip) → AST-гейт (статика) → smoke-импорт в изолированном subprocess (rlimits), нужен ≥1 `@tool` → только при успехе копия в `src/skills/` + запись в `registry.json`. Недоверенный код в процессе агента не исполняется.
